...
class MyHighlighter(QSyntaxHighlighter):
	def __init__(self, parent):
		...
		rule = HighlightingRule("#.*", comment, False)
		self.highlightingRules.append(rule)
	...
	def highlightBlock(self, text):
		...
		for rule in self.highlightingRules:
			if rule.weDontWantThis:
				...
				text = copy.deepcopy(originalText)
...
class CreateWidget(QWidget):
	...
	def clickedShowScale(self):
		if self.checkText():
			data, num, comments = parseInput(self.textBox.toPlainText())
			tones = getTones(data)
			paintTones(tones)
	...
	def clickedShowTabs(self):
		if self.checkText():
			...
			bounds, texts = computeAnnotations(comments)
			drawComments(Image, bounds, texts)
			saveImage(Image)
...
if __name__ == "__main__":
	...
	main = MainWindow()
	main.setFixedSize(620, 400) #setMinimumSize
